...
class UAV:
    ...
    def move(self, action: Tuple[float, float, float] = None, CollisionFlag = 0) -> None:
        if action == None:
            x_change, y_change, z_change = self.rng.choice(self.uav_action_lst)
        else:
            x_change, y_change, z_change = action

        position_old = np.array([self.x, self.y, self.z])
        position = position_old + [x_change, y_change, z_change]
        # CollisionFlag = 0
        if position[0] > (self.width - self.search_banjing_max) or position[0] < self.search_banjing_max \
            or position[1] > (self.height - self.search_banjing_max) or position[1] < self.search_banjing_max:
            CollisionFlag += 1
            position = position_old.copy()
        # position[0:2] = np.clip(position[0:2], self.search_banjing_max, max(self.width, self.height) - self.search_banjing_max - 1)
        position[2] = np.clip(position[2], self.h_min, self.h_3d)
        return tuple(position), CollisionFlag
