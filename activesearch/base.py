...
class Xia1(gymnasium.Env):
    def __init__(self, custom_config=None) -> None:
        super().__init__()
        config, uav_config, people_config = self.default_config()
        if custom_config:
            config.update(custom_config.get("env_config", {}))
            uav_config.update(custom_config.get("uav_config", {}))
            people_config.update(custom_config.get("people_config", {}))
        ...
        self.observation_space = gymnasium.spaces.Box(low=-1.0, high=1.0, shape=(config["observation_size"],))
        self.config = config
    ...
    def reset(self, *, seed: Union[int, None] = None, options: Union[Dict[str, Any], None] = None) -> Tuple[Any, Dict[str, Any]]:
        ...
        self.BeliefMap = np.ones((self.config["width"], self.config["height"])) * 0.5
    ...
    def BMapChange(self, uav):
        ...
        B#####
        '''
        search_banjing = int(uav.z * self.config['search_banjing_max'] / self.config['h_3d'])
        ...
        noise = np.random.normal(uav.z/1000.0, uav.z/200, size=AgT0.shape) * (-AgT)
        ...
        posterior_prob = (observation_prob * prior_prob) / \
                             ((observation_prob * prior_prob) + ((1 - observation_prob) * (1 - prior_prob)) + 1e-7)
    ...
    def calculate_entropy(self, belief_map):
        # 计算信息熵
        entropy = -np.sum(belief_map * np.log(belief_map + 1e-7))  # 加入小量避免对0取对数
        return entropy

    def _normalize_reward(self, rewards):
        # Normalize rewards to be in [-1, 1].
        min_r = -10000
        max_r = 10000
        norm_rewards = np.clip(rewards, -10000, 10000)
        norm_rewards = 2 * (norm_rewards - min_r) / (max_r - min_r) + -1.0
        return norm_rewards
    ...
    def _get_reward(self):  # Modify
        ...
        Factor = self.groundTruth0 * (10 - 1) + self.groundTruth   # 9,0矩阵+1，-1矩阵=10，-1矩阵
        # self.rew2 = np.sum(BMap_difference * self.groundTruth)
        self.rew2 = np.sum(BMap_difference * Factor)

        # reward = self.rew1 + self.rew2
        self.rew3 = 0
        ...
        reward = self.rew1 + self.rew2 + self.rew3
    ...
    def render(self) -> None:
        ...
        if self.time > 1:
            for i in range(self.time - 1):
                self.ax.plot3D([self.UAV_traj_render[i*2][0], self.UAV_traj_render[(i+1)*2][0]],
                               [self.UAV_traj_render[i*2][1], self.UAV_traj_render[(i+1)*2][1]],
                ...
                self.ax.plot3D([self.UAV_traj_render[i*2+1][0], self.UAV_traj_render[(i+1)*2+1][0]],
                               [self.UAV_traj_render[i*2+1][1], self.UAV_traj_render[(i+1)*2+1][1]],
        ...
        for i in self.ObsDict['_People_location']:
            self.ax1.scatter(int(i[1]), int(i[0]), marker='.', s=1, color="red")
        for i in self.ObsDict['_UAV_location']:
